...
- Windows Korean IME reports letter shortcuts with `KeyboardEvent.key === "Process"`; keep the one-task editing-host suspension for that path so non-cancelable composition does not start against inserted emote images.
